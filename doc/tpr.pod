...
This is one of the oldest Perl idioms, which has existed
right from the very beginning of the language.  When Perl was
first created, it was a much nicer way of doing error handling
than what was required in C, but that doesn't mean it's a good
way of doing error handling I<today>.
...
space than the function it is designed to guard.  One of the great
virtues of a programmer is I<laziness>, and yet Perl is a language
that requires us to check every system call.  The lazy programmer,
who leaves out these checks, ends up with buggy code.  Despite
the I<do or die> construct having been with Perl from the very
start, I consider it one of the most un-Perlish ways of doing
things.

Wouldn't it be nice if Perl's built-ins could just
automatically die on failure?
...
C<or die> ourselves, and it means the lazy usage (not
checking our system calls) becomes the I<correct> usage.

Using C<Fatal> is even mentioned in Dr Conway's book,
I<Perl Best Practices>, and it can be invoked like this:
...
Despite C<Fatal>'s long history in the Perl core, and apparently
simple usage, it suffers from some serious problems.  Since built-ins
and subroutines need to be explicitly listed, it's possible to end up
with very long and cumbersome use lines:
...
In fact, if you look closely, a large number of Perl
...
appropriate calls in the current package after the C<use Fatal>
line is invoked.  Unfortunately, for a large package, this can result
in difficult action-from-a-distance.  The code below contains a bug,
as C<use_default_config()> is never reached.  The C<use Fatal> line
deep inside the subroutine changes all calls to C<open> in the same
package, even the ones called above it.
    ...
    # Even though it doesn't immediately look like it, the following
...
In Perl, laziness is a virtue.  Fatal allows one to write code in
a lazy fashion, without having to manually check every single system
call for errors.  At least, that's what it's I<supposed> to do; as we've
